...
export default function SubscriptionLifecycle() {
  return (
    <>
      <p>
        A subscription isn&apos;t just &quot;active&quot; or &quot;not
        active.&quot; It moves through a defined state machine, and every
        transition is explicit, nothing about a subscription&apos;s status is
        inferred after the fact from a pile of payment records. This page is the
        whole machine.
      </p>
...
`}
      ...
      <p className="body-secondary">
        Note the one overload in this machine:{" "}
        <code className="inline">suspended</code> is reached two different ways,
        either a merchant-initiated pause, or a grace period that lapsed with no
        successful retry.
      </p>
      ...
      <table>
        <tbody>
          ...
          <tr>
            ...
            <td>
              Created, not yet activated, usually waiting on card tokenisation
              to complete.
            </td>
          </tr>
          <tr>
            ...
            <td>
              Inside the plan&apos;s trial window. No charge has fired yet.
            </td>
          </tr>
          <tr>
            <td>
          ...
          <tr>
            ...
            <td>
              A charge just failed. The clock on the grace period starts here.
            </td>
          </tr>
          <tr>
            <td>
            ...
            <td>
              Recovery is actively in progress, messages have gone out, retries
              are expected.
            </td>
          </tr>
          <tr>
            ...
            <td>
              Either a grace period that lapsed without recovery, or a
              merchant-initiated pause, both land here. Access is paused, but
              the subscription record and history are intact either way.
            </td>
          </tr>
          <tr>
            <td>
          ...
          <tr>
            ...
            <td>
              A fixed-term subscription reached its natural end. Terminal.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <h2 id="h-why-split">
        Why past_due and grace_period are separate states
      </h2>
      <p>
        It would be simpler to collapse these into one &quot;payment
        failed&quot; state. They&apos;re kept separate because they mean
        different things to a merchant reading the dashboard:{" "}
        <code className="inline">past_due</code> is a fact (a charge failed),{" "}
        <code className="inline">grace_period</code> is a process (recovery is
        actively running, with a defined end). A merchant scanning for
        subscriptions that need manual attention wants to see the ones where
        recovery has stalled, not every subscription that&apos;s ever had a
      ...
      <CardGrid cols={2}>
        ...
        <Card icon={Repeat} title="past_due → grace_period">
          Recovery orchestration picks up the failure and starts sending
          recovery messages.
        </Card>
        ...
        <Card icon={Ban} title="grace_period → suspended">
          The grace period lapses with no successful retry, the third dunning
          attempt has also failed.
        </Card>
        <Card icon={Repeat} title="active → suspended → active">
          A merchant pauses a subscription (billing stops) and later resumes it,
          the same status value dunning exhaustion lands on, reused rather than
          duplicated.
        </Card>
      ...
      <p>
        Each state change writes a domain event to the{" "}
        <a href="/concepts/event-store">event store</a>,{" "}
        ...
        <code className="inline">PaymentRecovered</code>. This is what lets
        webhooks, notifications, and analytics all react to the same state
        change without needing to poll the subscription record for a diff.
      </p>
