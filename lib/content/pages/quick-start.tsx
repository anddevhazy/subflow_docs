...
export const meta: PageMeta = {
  eyebrow: "Get started",
  title: "Quick start",
  lede: "Five real API calls: create a key, a plan, a customer, a subscription, and a webhook. ",
};
...
export default function QuickStart() {
  return (
    <>
      <p>
        Every call below hits the real API. Responses are wrapped in the
        standard envelope,{" "}
        <code className="inline">{"{ status, data, message }"}</code>, trimmed
        here to the parts that matter.
      </p>

      <Steps>
        <Step number={1} title="Sign in, then create a test API key">
          <CodeBlock
            code={`curl -X POST https://subscription-billing-engine-t7ss.onrender.com/auth/login \\
  -H "Content-Type: application/json" \\
  -d '{"email": "founder@acme.ng", "password": "securePass123"}'

curl -X POST https://subscription-billing-engine-t7ss.onrender.com/api-keys \\
  ...
  "message": "Resource created successfully"
          ...
          <p className="body-secondary">
            Save <code className="inline">rawKey</code> now, it&apos;s shown
            once. See <a href="/developer/authentication">Authentication</a>.
          </p>
        </Step>
  ...
  -d '{
    ...
    "trialDays": 14
          ...
          />
          <p className="body-secondary">
            Amounts are major currency units, ₦15,000 is 15000, not kobo.
          </p>
        </Step>

        <Step number={3} title="Create a customer">
          <CodeBlock
            code={`curl -X POST https://subscription-billing-engine-t7ss.onrender.com/customers \\
  -H "Authorization: Bearer <access_token>" \\
  -H "Content-Type: application/json" \\
  -d '{
    "name": "Chidi Nwosu",
    ...
    "phone": "+2348012345678"
        ...
        <Step number={4} title="Create a subscription and redirect to checkout">
          <CodeBlock
            code={`curl -X POST https://subscription-billing-engine-t7ss.onrender.com/subscriptions \\
  -H "Authorization: Bearer <access_token>" \\
  -H "Content-Type: application/json" \\
  -d '{
  ...
  "message": "Resource created successfully"
          ...
          <p className="body-secondary">
            ...
            <code className="inline">checkoutUrl</code>. Monnify confirms the
            outcome by webhook, not by the checkout redirect alone, before the
            subscription moves to <code className="inline">trialing</code> or{" "}
            <code className="inline">active</code>. See{" "}
  ...
  -d '{
    ...
    "events": ["invoice.paid", "payment.failed", "payment.recovered"]
          ...
          <CodeBlock
            code={`curl https://subscription-billing-engine-t7ss.onrender.com/analytics/metrics \\
  -H "Authorization: Bearer <access_token>"`}
            language="bash"
          />
          <p className="body-secondary">
            MRR, churn, and recovery rate, computed from the same event store
            your webhook just read from, rate fields are 0-100 percentages, not
            fractions.
          </p>
        </Step>
      </Steps>
